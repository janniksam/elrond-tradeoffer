...
namespace Elrond.TradeOffer.Web.Services
{
    public class TransactionGenerator : ITransactionGenerator
    {
        ...
        {
            var offerCreator = await _userManager.GetAsync(offer.CreatorUserId, CancellationToken.None);
            var bidder = await _userManager.GetAsync(acceptedBid.CreatorUserId, CancellationToken.None);
            if (bidder.Address == null)
            {
                throw new ArgumentException("Address of the user needs to be filled.", nameof(bidder));
            }

            var networkStrategy = _networkStrategies.GetStrategy(offerCreator.Network);
            var networkConfig = await _elrondApiService.GetNetworkConfigAsync(offerCreator.Network);
        ...
        private static string CreateDataInitiateTradeForEgld(Bid bid)
        {
            ...
            var tokenWant = bid.Amount.Token.Id.ToHex();
        ...
        {
            var offerId = offer.Id.ToHex();
            var tokenWant = offer.Amount.Token.Id.ToHex();
        ...
        private static string CreateDataInitiateTradeForEsdt(string scAddress, Offer offer, TokenAmount want)
        {
            ...
            var tokenOffered = offer.Amount.Token.Id.ToHex();
            ...
            var tokenWant = want.Token.Id.ToHex();
        ...
        private static string CreateDataFinalizeTradeForEsdt(string scAddress, Bid acceptedBid, TokenAmount want)
        {
            ...
            var receiver = scAddress.FromBech32ToHex();
            var tokenBid = acceptedBid.Amount.Token.Id.ToHex();
